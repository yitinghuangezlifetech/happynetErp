...
<form id="form" enctype="multipart/form-data" method="POST" action="{{route($menu->slug.'.update', $data->id)}}" onsubmit="return checkSign()">
                  ...
                  @else
                    ...
                    @else
                      @if(count($json) > 0)
                        ...
                        @endif
                      @else
                        @if($detail->field == 'company_seal' || $detail->field == 'company_stamp' || $detail->field == 'sender_sign' || $detail->field == 'customer' || $detail->field == 'recipient_sign')
                        <div class="col-sm-6">
                          <div class="form-group">
                            <label>{{$detail->show_name}}</label>
                            <img src="{{ $data->{$detail->field} }}" style="float: right">
                          </div>
                        </div>
                        @else
  ...
  <div class="card" id="footerArea">
    <div class="card-footer text-center">
      <button type="submit" class="btn bg-gradient-dark">確認</button>
...
@section('js')
  ...
  <script>
  const checkSign = () => {
    const rows = parseInt($('#auditor_sign_area img').length);
    const status = $('#edit_status').val();
    const reason = $('#edit_fail_response').val();
  ...
  $(document).ready(function(){
    ...
    const init = () => {
      ...
      $('#edit_auditor_id').attr('disabled', false);
      $('#edit_status').attr('disabled', false);

      $('#edit_auditor_id').val('{{$user->id}}');
      ...
      $('#edit_status option').each(function(index){
        if ($(this).text() != '請選擇' && $(this).text() != '通過' && $(this).text() != '不通過') {
          $(this).remove();
        }
      })
    }
