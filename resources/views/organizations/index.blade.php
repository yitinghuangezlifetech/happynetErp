                    ...
                    <table class="table table-bordered text-nowrap">
                        <thead>
                            <tr>
                                ...
                                <th class="bg-gradient-secondary" style="text-align: center">費率表</th>
                        ...
                        <tbody>
                            @foreach ($list ?? [] as $data)
                                <tr>
                                    <td class="text-center"><input type="checkbox" class="rowItem" name="items[]"
                                            value="{{ $data->id }}"></td>
                                    <td style="text-align: center">
                                        @can('update_' . $menu->slug, app($menu->model))
                                            <button type="button" class="btn bg-gradient-secondary btn-sm"
                                                onclick="location.href='{{ route($menu->slug . '.edit', $data->id) }}'"><i
                                                    class="fas fa-edit"></i></button>
                                        @endcan
                                    </td>
                                    <td class="text-center" style="vertical-align: middle">{{ $data->system_no }}</td>
                                    <td class="text-center" style="vertical-align: middle">
                                        {{ optional($data->group)->name }}</td>
                                    <td class="text-center" style="vertical-align: middle">
                                        {{ optional($data->identity)->type_name }}</td>
                                    <td class="text-center" style="vertical-align: middle">{{ $data->name }}</td>
                                    <td class="text-center" style="vertical-align: middle">
                                        {{ optional($data->feeRate)->name }}</td>
                                    <td class="text-center" style="vertical-align: middle">{{ $data->id_no }}</td>
                                    <td class="text-center" style="vertical-align: middle">{{ $data->systemRoles->count() }}
                                    </td>
                                    <td class="text-center" style="vertical-align: middle">{{ $data->agentRoles->count() }}
                                    </td>
                                    <td class="text-center" style="vertical-align: middle">
                                        {{ $data->customerRoles->count() }}</td>
                                    <td class="text-center" style="vertical-align: middle">
                                        <input type="checkbox" class="switchBtn status" checkeddata-toggle="toggle"
                                            data-id="{{ $data->id }}" data-field="status"
                                            data-model="{{ $menu->model }}"
                                            @if ($data->status == 1) {{ 'checked' }} @endif>
                                    </td>
                                    <td class="text-center" style="vertical-align: middle">
                                        {{ optional($data->createUser)->name }}</td>
                                    <td class="text-center" style="vertical-align: middle">{{ $data->created_at }}</td>
                                    <td class="text-center" style="vertical-align: middle">
                                        {{ optional($data->updateUser)->name }}</td>
