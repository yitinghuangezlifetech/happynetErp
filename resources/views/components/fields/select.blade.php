    ...
    if ($detail->has_relationship == 1) {
        ...
        if (count($decodeData) > 0) {
            if (isset($decodeData['type_code'])) {
                $options = collect([]);
                $main = app($decodeData['model'])
                    ->where('type_code', $decodeData['type_code'])
                    ->first();
...
<div class="form-group">
    <label
        @if ($type == 'edit') for="edit_{{ $detail->field }}"
        @else
        for="{{ $detail->field }}" @endif />
    @if ($detail->required == 1 && $type != 'search')
    ...
    <select class="form-control" name="{{ $detail->field }}"
        @if ($type == 'edit') id="edit_{{ $detail->field }}"
        @else
        id="{{ $detail->field }}" @endif
        {!! $str !!} @if ($detail->required == 1 && $type != 'search') {{ 'required' }} @endif />
