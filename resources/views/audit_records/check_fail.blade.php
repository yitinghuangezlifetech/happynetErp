...
@section('content')
<form method="post" enctype="multipart/form-data" action="{{route('pending_checks.failRecord')}}" onsubmit="return checkFailItems()">
    @csrf
                ...
                <div class="col-md-12">
                    <table class="table">
                        <tbody id="fialContent">
                            @if($record->getFailLogsByLogId($checkLog->id)->count() > 0)
                                @foreach($record->getFailLogsByLogId($checkLog->id) as $key=>$fail)
                                ...
                                <tr class="row_{{$fail->id}}">   
                                    <td>
                                        <select name="fails[{{$fail->id}}][regulation_fact_id]" class="form-control" data-number="{{$record->regulation->no}}" class="item">
