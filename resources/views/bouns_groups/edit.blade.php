        ...
        <tbody>
          @foreach($funcTypes??[] as $type)
            ...
            @endif
              ...
              <td>
                <input type="hidden" name="bonus[{{$loop->iteration}}][func_type_id]" value="{{$type->id}}">
                <input type="text" class="form-control" name="bonus[{{$loop->iteration}}][bonus]" value="{{$percent}}">
              </td>
            @if($loop->last)
            </tr>  
            @endif
