            ...
            <div class="card">
                <div class="card-header">
                    @can('create_bouns_groups', App\Models\BounsGroup::class)
                        <button type="button" class="btn bg-gradient-secondary btn-sm" style="float: left; margin-left: 5px;"
                            onClick="location.href='{{ route('bouns_groups.create') }}'">建立奬金群組資料</button>
                    @endcan
                    <button type="button" class="btn bg-gradient-secondary btn-sm closeMenu"
                        style="float: left; margin-left: 5px;" onclick="closeMenu()">全部收合</button>
                    <button type="button" class="btn bg-gradient-secondary btn-sm openMenu"
                        style="float: left; margin-left: 5px; display: none" onclick="openMenu()">全部打開</button>
                </div>
                <div class="card-body">
                                ...
                                <li class="dd-item" data-id="{{ $data->id }}">
                                    <div class="dd-handle" style="vertical-align: middle">
                                        ...
                                        <div class="btn-group m-b-10 chili-btn-group" style="float: right!important;">
                                            <i class="fas fa-edit btn" title="編輯"
                                                onmousedown="location.href='{{ route('bouns_groups.edit', $data->id) }}'"></i>
                                            <i class="fas fa-trash-alt btn delBtn" title="刪除"
                                                data-id="{{ $data->id }}"
                                                onmousedown="menuDelete('{{ $data->id }}')"></i>
                                        </div>
                                    </div>
