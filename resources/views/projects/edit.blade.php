        ...
        <div class="card card-secondary">
            <div class="card-header">
                ...
                <div style="float: right;">
                    <button type="button" class="btn btn-block btn-outline-secondary btn-sm addProducts"
                        style="color:white"><i class="fas fa-plus-circle"></i> 增加商品</button>
                </div>
            </div>
                    ...
                    <div class="card card-secondary" id="car_{{ $loop->iteration }}" style="margin-top: 10px;">
                                ...
                                <table>
                                    <tr>
                                        <td><button type="button"
                                                class="btn btn-block btn-outline-secondary btn-sm removeProduct"
                                                style="color:white;" data-row="{{ $loop->iteration }}"><i
                        ...
                        <div class="card-body">
                            <div class="form-group row">
                                ...
                                <select class="custom-select form-control-border productType"
                                    name="products[{{ $loop->iteration }}][product_type_id]"
                                    id="product_type_id_{{ $loop->iteration }}" data-row="{{ $loop->iteration }}"
                                    required>
                                    <option value="">請選擇商品類別</option>
                                    @foreach ($types ?? [] as $type)
                                        <option value="{{ $type->id }}"
                                            @if ($type->id == $k) {{ 'selected' }} @endif>
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($products ?? [] as $product)
                                            ...
                                            <td style="text-align: left;max-width:20%; width:20%">
                                                <input type="checkbox" name="products[{{ $row }}][items][]"
                                                    value="{{ $product->id }}"
                                                    @if (isset($logs[$k][$product->id])) {{ 'checked' }} @endif>
                                                {{ $product->name }}
                                            </td>
            ...
            <div class="card card-secondary">
                <div class="card-header">
                    ...
                    <div style="float: right;">
                        <button type="button" class="btn btn-block btn-outline-secondary btn-sm addRegulation"
                            style="color:white"><i class="fas fa-plus-circle"></i> 增加條文</button>
                    </div>
                </div>
                <div class="card-body" id="regulationArea">
                    @foreach ($data->terms ?? [] as $log)
                        <div class="card card-secondary disabled" id="regulation_car_{{ $loop->iteration }}"
                            style="margin-top: 10px;">
                            <div class="card-header">
                                <h3 class="card-title main-title"><i class="fas fa-arrows-alt"
                                        style="cursor:pointer"></i>&nbsp;&nbsp;條文{{ $loop->iteration }}</h3>
                                <div style="float: right;">
                                    <table>
                                        <tr>
                                            <td><button type="button"
                                                    class="btn btn-block btn-outline-secondary btn-sm removeRegulation"
                                                    style="color:white;" data-row="{{ $loop->iteration }}"><i
                                                        class="fas fa-trash-alt"></i>&nbsp;刪除</button></td>
                                        </tr>
                                    </table>
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        <div class="form-group" id="regulationItem_{{ $loop->iteration }}">
                                            ...
                                            <input type="hidden" name="regulations[{{ $loop->iteration }}][sort]"
                                                id="sort_{{ $log->sort }}" value="{{ $log->sort }}">
                                            <input type="hidden" class="term_id"
                                                name="regulations[{{ $loop->iteration }}][term_id]"
                                                value="{{ $log->term_id }}">
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
            ...
            new Sortable(regulationArea, {
                ...
                onEnd: function( /**Event*/ evt) {
                    ...
                    $('#regulationArea .main-title').each(function() {
                        $(this).html(
                            `<i class="fas fa-arrows-alt" style="cursor:pointer"></i>&nbsp;&nbsp;條文${number}`
                        );
                        number++;
                    })
        ...
        `;
            ...
            $('body').on('click', '.removeProduct', function() {
                ...
                let productId = $(`#product_type_id_${row}`).val();

                productTypeArr = productTypeArr.filter(id => id !== productId);
        ...
        `;
            ...
            $('body').on('change', '.regulationProductType', function() {
                const id = $(this).val();
                const row = $(this).data('row');
                const saleTypeId = $('#sales_type_id').val();
                const termTypeId = $(`#term_type_id_${row}`).val();
            ...
            $('body').on('click', '.removeRegulation', function() {
                ...
                $(`#regulationItem_${row} .term_id`).each(function() {
                    if ($(this).prop('type') == 'checkobx') {
                        if ($(this).prop('checked')) {
                            terms = terms.filter(id => id != $(this).val());
                        }
                    } else {
                        terms = terms.filter(id => id != $(this).val());
                    }
