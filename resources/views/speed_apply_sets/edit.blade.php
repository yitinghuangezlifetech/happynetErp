    ...
    <form id="form" enctype="multipart/form-data" method="POST" action="{{ route($menu->slug . '.update', $data->id) }}">
        ...
        <div class="card card-secondary">
            <div class="card-header">
                ...
                <div style="float: right;">
                    <button type="button" class="btn btn-block btn-outline-secondary btn-sm addSubject"
                        style="color:white"><i class="fas fa-plus-circle"></i> 增加項目</button>
                </div>
            </div>
            <div class="card-body" id="subjectContent">
                @foreach ($data->contents ?? [] as $k => $content)
                    <div class="card card-secondary disabled" id="car_{{ $k }}" style="margin-top: 10px;">
                        <input type="hidden" name="field[{{ $k }}][sort]" id="sort_{{ $k }}"
                            value="{{ $content->sort }}">
                        <div class="card-header">
                                ...
                                <table>
                                    <tr>
                                        <td><button type="button"
                                                class="btn btn-block btn-outline-secondary btn-sm removeSubject"
                                                style="color:white;" data-row="{{ $k }}"><i
                                                    class="fas fa-trash-alt"></i>&nbsp;刪除</button></td>
                                    </tr>
                                </table>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="form-group row">
                                ...
                                <select class="custom-select form-control-border topicType"
                                    name="field[{{ $k }}][field_attribute]"
                                    id="field_attribute_{{ $k }}" data-row="{{ $k }}" required>
                                    <option value="">請選擇欄位屬性</option>
                                    @foreach ($types ?? [] as $type)
                                        <option value="{{ $type->val }}"
                                            @if ($content->field_attribute == $type->val) {{ 'selected' }} @endif>
                                            {{ $type->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group row">
                                ...
                                <input type="text" class="form-control" name="field[{{ $k }}][title]"
                                    id="subject_{{ $k }}" value="{{ $content->title }}" placeholder="請輸入欄位名稱"
                                    required>
                            </div>
                            <div id="options_{{ $k }}"
                                @if ($content->items->count() == 0) style="display:none" @endif>
                                <div class="form-group row">
                                    <label>選項設定</label>
                                </div>
                                <div class="form-group row content_{{ $k }}">
                                    @foreach ($content->items ?? [] as $k1 => $item)
                                        <div class="col-sm-12" id="item_{{ $k }}_{{ $k1 }}"
                                            style="margin-top: 10px;">
                                            <div class="input-group">
                                                <input type="text" class="form-control"
                                                    name="field[{{ $k }}][items][{{ $k1 }}][name]"
                                                    value="{{ $item->name }}" placeholder="請輸入選項名稱" required>
                                                <div class="input-group-append">
                                                    <span class="input-group-text">
                                                        <button type="button" class="btn btn-xs multipleMinus"
                                                            data-row="{{ $k }}"
                                                            data-item="{{ $k1 }}"><i
                                                                class="fas fa-minus"></i></button>
                                                    </span>
                                                    <span class="input-group-text">
                                                        <button type="button" class="btn btn-xs multiplePlus"
                                                            data-row="{{ $k }}"><i
                                                                class="fas fa-plus"></i></button>
                                                    </span>
                                                    <span class="input-group-text">
                                                        <button type="button"
                                                            class="btn btn-block btn-outline-secondary btn-sm addChildSubject"
                                                            data-row="{{ $k }}"
                                                            data-item="{{ $k1 }}" style="color:black;"><i
                                                                class="fas fa-plus-circle"></i> 增加子項目</button>
                                                    </span>
                                                </div>
                                            </div>
                                            <div id="childSubject_{{ $k }}_{{ $k1 }}"
                                                @if ($item->childrens->count() == 0) style="display:none" @endif>
                                                @foreach ($item->childrens ?? [] as $k2 => $child)
                                                    @php $number = $k2 + 1;@endphp
                                                    <div class="card card-warning disabled"
                                                        id="sub_car_{{ $k }}_{{ $k1 }}_{{ $k2 }}"
                                                        style="margin-top: 10px;">
                                                        <div class="card-header">
                                                            ...
                                                            <div style="float: right;">
                                                                <table>
                                                                    <tr>
                                                                        <td><button type="button"
                                                                                class="btn btn-block btn-outline-default btn-sm removeChildSubject"
                                                                                style="color:red;"
                                                                                data-row="{{ $k }}"
                                                                                data-item="{{ $k1 }}"
                                                                                data-subrow="{{ $k2 }}"><i
                                                        ...
                                                        <div class="card-body">
                                                            <div class="form-group row">
                                                                <label
                                                                    for="field_attribute_{{ $k }}_{{ $k1 }}_{{ $k2 }}">欄位屬性</label>
                                                                <select
                                                                    class="custom-select form-control-border childTopicType"
                                                                    name="field[{{ $k }}][items][{{ $k1 }}][child][{{ $k2 }}][field_attribute]"
                                                                    id="field_attribute_{{ $k }}_{{ $k1 }}_{{ $k2 }}"
                                                                    data-row="{{ $k }}"
                                                                    data-item="{{ $k1 }}"
                                                                    data-subrow="{{ $k2 }}" required">
                                                                    <option value="">請選擇欄位屬性</option>
                                                                    @foreach ($types ?? [] as $type)
                                                                        <option value="{{ $type->val }}"
                                                                            @if ($child->field_attribute == $type->val) {{ 'selected' }} @endif>
                                                                            {{ $type->name }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                                            <div class="form-group row">
                                                                <label
                                                                    for="subject_{{ $k }}_{{ $k1 }}_{{ $k2 }}">欄位名稱</label>
                                                                <input type="text" class="form-control"
                                                                    name="field[{{ $k }}][items][{{ $k1 }}][child][{{ $k2 }}][title]"
                                                                    id="subject_{{ $k }}_{{ $k1 }}_{{ $k2 }}"
                                                                    value="{{ $child->title }}" placeholder="請輸入欄位名稱"
                                                                    required>
                                                            </div>
                                                            <div id="options_{{ $k }}_{{ $k1 }}_{{ $k2 }}"
                                                                @if ($child->items->count() == 0) style="display:none" @endif>
                                                                <div class="form-group row">
                                                                    <label>選項設定</label>
                                                                </div>
                                                                <div
                                                                    class="form-group row content_{{ $k }}_{{ $k1 }}_{{ $k2 }}">
                                                                    @foreach ($child->items ?? [] as $k3 => $item)
                                                                        <div class="col-sm-12"
                                                                            id="child_item_{{ $k }}_{{ $k1 }}_{{ $k2 }}_{{ $k3 }}"
                                                                            style="margin-top: 10px;">
                                                                            <div class="input-group">
                                                                                <input type="text" class="form-control"
                                                                                    name="field[{{ $k }}][items][{{ $k1 }}][child][{{ $k2 }}][items][{{ $k3 }}][name]"
                                                                                    value="{{ $item->name }}"
                                                                                    placeholder="請輸入選項名稱" required>
                                                                                <div class="input-group-append">
                                                                                    <span class="input-group-text">
                                                                                        <button type="button"
                                                                                            class="btn btn-xs childMultipleMinus"
                                                                                            data-row="{{ $k }}"
                                                                                            data-subrow="{{ $k2 }}"
                                                                                            data-item="{{ $k1 }}"
                                                                                            data-itemcount="{{ $k3 }}"><i
                                                                                    ...
                                                                                    <span class="input-group-text">
                                                                                        <button type="button"
                                                                                            class="btn btn-xs childMultiplePlus"
                                                                                            data-row="{{ $k }}"
                                                                                            data-subrow="{{ $k2 }}"
                                                                                            data-item="{{ $k1 }}"
                                                                                            data-itemcount="{{ $k3 }}"><i
        ...
        <div class="card" id="footerArea">
            <div class="card-footer text-center">
                ...
                <button type="button" class="btn bg-gradient-secondary preview"><i
                        class="fas fa-eye"></i>&nbsp;預覽</button>
                <button type="button" class="btn bg-gradient-secondary deleteBtn"><i
                        class="fas fa-trash-alt"></i>&nbsp;刪除</button>
    ...
    <script>
      ...
      `;
            ...
            $('body').on('change', '.childTopicType', function() {
                ...
                var item = $(this).data('item');
                var subRow = $(this).data('subrow');
                var itemCount = parseInt($(`.content_${row}_${item}_${subRow} div[id^="child_item_"]`)
                    .length);
                var str = '';
